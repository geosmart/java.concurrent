...
## 守护线程(Daemon Thread)
1. 线程可分两种：普通线程和守护线程；
2. 在JVM启动时创建的所有线程中，除了主线程以外，其他线程都是守护线程（如垃圾回收器以及其他执行辅助工作的线程）；
3. 当创建一个新线程时，新线程将继承创建它的线程的守护状态，因此在默认情况下，主线程创建的所有线程都是普通线程；
4. 普通线程和守护线程之间的差异在于当线程退出时发生的操作：
    * 当一个`线程退出`时，JVM会检查其他正在运行的线程，如果这些线程都是`守护线程`，那么JVM会`正常退出`操作；
    * 当`JVM停止`时，所有仍然存在的守护线程都将被`抛弃`--既不会执行finally代码块，也不会执行回卷栈，而JVM只是直接退出；
5. `尽可能少地使用守护线程`，因为很少有操作能够在不进行清理的情况下被安全的抛弃，特别是如果守护线程在执行I/O操作的任务，那么将是一种危险的行为；
6. 守护线程最好用于执行`内部任务`，例如周期性的从内存的缓存中移除逾期的数据；

## 终结器
1. 当不再需要内存资源时，可以通过垃圾回收器来回收它们，但对于其他一些资源，如`文件句柄`或`套接字句柄`，当不再需要它们时，必须`显式`的交还给操作系统。
2. `垃圾回收器`对那些定义了`finalize`方法的对象会进行特殊处理：在`回收器释放`它们后，调用它们的finalize方法，从而保证一些持久化的资源被释放；
3. 由于终结器可以在某个由JVM管理的线程中运行，因此终结器访问的任何状态都可能被多个线程访问，这样就必须对其访问操作进行同步；
4. 终结器并不能保证它们在何时运行甚至是否会运行，并且复杂的终结器通常还会在对象上产生巨大的性能开销；
5. 要编写正确的终结器是很困难的，大多数情况下，通过使用`finally代码块`或显式的`close方法`，能够比终结器更好的管理资源；
6. 唯一例外的情况，当需要管理对象，并且`该对象持有的资源是通过本地方法获得的`，基于这些原因我们要尽量避免编写或使用包含终结器的类（除非是平台库中的类）；

# 小结
1. 在任务、线程、服务以及应用程序等模块中的`生命周期结束问题`，可能会增加它们在设计和实现时的复杂性；
2. Java并没有提供某种抢占式的机制来`取消操作`或者`终结线程`。相反，它提供了一种`协作式的中断机制`来实现取消操作，但这要依赖于如何构建取消操作的协议，以及是否遵循这些协议；
3. 通过使用`FutureTask`和`Executor`框架，可以帮助我们构建`可取消的任务和服务`；